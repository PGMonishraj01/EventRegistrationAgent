...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tabMap = {
        ...
        profile: {
            button: document.getElementById('tab-btn-profile'),
            view: document.getElementById('view-profile'),
            onEnter: loadUserProfile
        }
    };
    ...
    async function sendMessage() {
        ...
        try {
            ...
            // After RECOMMEND response, inject clickable event cards
            if (data.type === null || data.type === undefined || data.type === 'RECOMMENDATION') {
                await tryInjectEventCards(data.response);
            }
    ...
    // Try to parse event names from RECOMMEND response and show clickable cards
    async function tryInjectEventCards(responseText) {
        ...
        try {
            ...
            // Find events mentioned in AI response
            const mentionedEvents = allEvents.filter(ev =>
                responseText.toLowerCase().includes(ev.name.toLowerCase())
            );

            if (mentionedEvents.length === 0) return;

            // Build cards container
            const cardsWrapper = document.createElement('div');
            cardsWrapper.className = 'chat-event-cards-container';

            const label = document.createElement('p');
            label.className = 'chat-event-cards-label';
            label.textContent = '👆 Click a card to register for that event instantly:';
            cardsWrapper.appendChild(label);

            const cardsRow = document.createElement('div');
            cardsRow.className = 'chat-event-cards-row';

            mentionedEvents.forEach(ev => {
                const card = document.createElement('div');
                card.className = 'chat-event-suggestion-card';
                card.setAttribute('role', 'button');
                card.setAttribute('tabindex', '0');
                card.setAttribute('title', `Click to register for ${ev.name}`);
                card.innerHTML = `
                    <div class="suggestion-card-category">${escapeHtml(ev.category || 'Event')}</div>
                    <div class="suggestion-card-name">${escapeHtml(ev.name)}</div>
                    <div class="suggestion-card-meta">
                        <span>📅 ${escapeHtml(ev.eventDate || 'TBA')}</span>
                        <span>📍 ${escapeHtml(ev.venue || 'Online')}</span>
                    </div>
                    <div class="suggestion-card-footer">
                        <span class="suggestion-card-fee">${Number(ev.registrationFee) === 0 ? 'Free' : '₹' + ev.registrationFee}</span>
                        <span class="suggestion-card-seats">${ev.availableSeats} seats left</span>
                    </div>
                    <div class="suggestion-card-cta">Register Now →</div>
                `;

                const openModal = () => {
                    // Get dummy trigger button
                    const dummy = { disabled: false, textContent: 'Register' };
                    showRegistrationModal(ev, dummy);
                };
            ...
            chatContainer.appendChild(cardsWrapper);
            chatContainer.scrollTop = chatContainer.scrollHeight;
        } catch (err) {
            console.error('Failed to inject event cards:', err);
        }
    }

    // Load user profile into profile form
    ...
    function setProfileFormValues(profile) {
        ...
        set('profile-phone', profile.phoneNumber);
        set('profile-address', profile.address);
        set('profile-interests', profile.interests);
        set('profile-age', profile.age);
        set('profile-usertype', profile.userType);
        set('profile-industry', profile.industry);
    }

    // Profile form submit handler (attached once)
    ...
    if (profileFormEl) {
        profileFormEl.addEventListener('submit', async (e) => {
            ...
            const submitBtn = profileFormEl.querySelector('.profile-save-btn');
            submitBtn.disabled = true;
            ...
            const dto = {
                ...
                userType: document.getElementById('profile-usertype').value.trim(),
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    Object.assign(storedUser, result);
                    localStorage.setItem('user', JSON.stringify(storedUser));
                    // Update header display
                    if (launchpadUserName) launchpadUserName.textContent = result.fullName || storedUser.fullName;
                    const userNameEl = document.getElementById('user-name');
                    if (userNameEl) userNameEl.textContent = result.fullName || storedUser.fullName;
                } else {
                    profileStatus.textContent = '❌ ' + (result.message || 'Failed to save profile.');
                    profileStatus.className = 'profile-status error';
                }
            } catch (err) {
                profileStatus.textContent = '❌ Connection error. Please try again.';
                profileStatus.className = 'profile-status error';
            } finally {
                submitBtn.disabled = false;
                submitBtn.textContent = 'Save Profile';
                setTimeout(() => { profileStatus.textContent = ''; profileStatus.className = 'profile-status'; }, 4000);
            }
        });
    }
...
function showRegistrationModal(eventItem, triggerButton) {
    ...
    // Pre-fill values from localStorage user profile
    const storedUser = JSON.parse(localStorage.getItem('user') || '{}');
    const prefillName = storedUser.fullName || '';
    const prefillPhone = storedUser.phoneNumber || '';

    const modal = document.createElement('div');
    modal.className = 'ticket-modal-overlay';
    modal.innerHTML = `
        <div class="ticket-modal-card reg-modal-card" role="dialog" aria-modal="true" aria-label="Register for ${escapeHtml(eventItem.name)}">
            <button type="button" class="ticket-modal-close" id="ticket-modal-close">×</button>
            <div class="ticket-modal-header reg-modal-header">
                <div class="reg-modal-badge">${escapeHtml(eventItem.category || 'Event')}</div>
            ...
            <div class="ticket-modal-body">
                <div class="ticket-modal-summary reg-event-summary">
                    <div class="reg-summary-item"><span class="reg-summary-icon">📅</span><div><strong>Date</strong><span>${escapeHtml(eventItem.eventDate || 'TBA')}</span></div></div>
                    <div class="reg-summary-item"><span class="reg-summary-icon">🕐</span><div><strong>Time</strong><span>${escapeHtml(eventItem.eventTime || '10:00 AM')}</span></div></div>
                    <div class="reg-summary-item"><span class="reg-summary-icon">📍</span><div><strong>Venue</strong><span>${escapeHtml(eventItem.venue || 'Online')}</span></div></div>
                    <div class="reg-summary-item"><span class="reg-summary-icon">🎫</span><div><strong>Seats Left</strong><span>${escapeHtml(String(eventItem.availableSeats ?? 0))}</span></div></div>
                    <div class="reg-summary-item"><span class="reg-summary-icon">💰</span><div><strong>Fee</strong><span>${Number(eventItem.registrationFee) === 0 ? 'Free' : '₹' + eventItem.registrationFee}</span></div></div>
                    <div class="reg-summary-item"><span class="reg-summary-icon">👥</span><div><strong>Audience</strong><span>${escapeHtml(eventItem.targetAudience || 'All')}</span></div></div>
                </div>
                <form id="registration-ticket-form" class="ticket-modal-form reg-modal-form">
                    <div class="reg-form-section-title">Personal Information</div>
                    <div class="reg-form-row">
                        <label>
                            Full Name *
                            <input type="text" id="ticket-attendee-name" required placeholder="Enter your full name" value="${escapeHtml(prefillName)}">
                        </label>
                        <label>
                            Contact Number *
                            <input type="tel" id="ticket-contact-number" required placeholder="e.g. +91 98765 43210" value="${escapeHtml(prefillPhone)}">
                        </label>
                    </div>
                    <div class="reg-form-row">
                        <label>
                            Email Address *
                            <input type="email" id="ticket-email" required placeholder="your@email.com" value="${escapeHtml(storedUser.email || '')}">
                        </label>
                        <label>
                            Age
                            <input type="number" id="ticket-age" min="10" max="100" placeholder="Your age" value="${escapeHtml(String(storedUser.age || ''))}">
                        </label>
                    </div>
                    <label>
                        Address / Location
                        <textarea id="ticket-address" rows="2" placeholder="City, State, Country">${escapeHtml(storedUser.address || '')}</textarea>
                    </label>

                    <div class="reg-form-section-title" style="margin-top:1rem;">Booking Details</div>
                    <div class="reg-form-row">
                        <label>
                            Number of Tickets *
                    ...
                    <div class="ticket-modal-actions" style="margin-top:1.2rem;">
                        <button type="button" class="ticket-modal-cancel" id="ticket-modal-cancel">Cancel</button>
                        <button type="submit" class="ticket-modal-submit" id="ticket-modal-submit">
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const contactNumber = modal.querySelector('#ticket-contact-number').value.trim();
        const emailVal = modal.querySelector('#ticket-email').value.trim();
        ...
        if (!contactNumber) {
            status.textContent = 'Please enter your contact number.';
            status.className = 'ticket-modal-status error';
            return;
        }
        ...
        submitButton.innerHTML = '<span>⏳</span> Processing...';
        ...
        try {
            ...
            preview.innerHTML = `
                <div class="reg-success-banner">
                    ...
                    <div>
                        ...
                        <div>Registration ID: <code>${escapeHtml(registrationId)}</code></div>
                    </div>
                </div>
                <img src="${ticketDataUrl}" alt="Event ticket preview" class="ticket-preview-image">
            `;
            const downloadLink = document.createElement('a');
            downloadLink.href = ticketDataUrl;
            ...
            status.textContent = '✅ Ticket generated and downloaded!';
            if (triggerButton && typeof triggerButton.disabled !== 'undefined') {
            ...
            submitButton.innerHTML = '<span>🎫</span> Complete Registration';
        }
    });
}
